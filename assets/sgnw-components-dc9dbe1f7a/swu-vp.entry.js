import { r as registerInstance, h, H as Host, g as getElement } from './index-DPMxB3IU.js';
...
*/
var swu = swu$1.exports;

var hasRequiredSwu;

function requireSwu () {
	if (hasRequiredSwu) return swu$1.exports;
	hasRequiredSwu = 1;
	(function (module, exports) {
		(function (global, factory) {
		  factory(exports) ;
		})(swu, (function (exports) {
		  /**
...
var swuExports = requireSwu();

const swuVpCss = () => `:host{writing-mode:vertical-lr;height:100%}:host span.outside{font-size:0%;line-height:0%;vertical-align:top}:host span.middle{vertical-align:bottom}swu-sign,swu-symbol{writing-mode:horizontal-tb;display:inline-block;vertical-align:middle;box-sizing:content-box}.text{writing-mode:horizontal-tb;display:inline-block;font-size:14px;line-height:1.2;padding:4px 6px}`;

const SwuVp = class {
    constructor(hostRef) {
        registerInstance(this, hostRef);
        /** Scale factor for layout metrics (default `1`). */
        this.zoom = 1;
        /** Layout constant for lane spacing (default `150`). */
        this.laneOffset = 150;
    }
    getValue() {
        const propValue = this.vp?.trim();
        if (propValue)
            return propValue;
    ...
    render() {
        ...
        let parts = [];
        try {
            parts = swuExports.parse.text(value);
        }
        catch {
            parts = value.split(/\s+/g).filter(Boolean);
        }
        const items = parts.map(part => {
            try {
                return { ...swuExports.info(part), text: part };
            }
            catch {
                return { segment: 'text', text: part };
            }
        });
        const offset = this.laneOffset;
        const baseZoom = Number.isFinite(this.zoom) ? this.zoom : 1;
        return (h(Host, { vp: value }, h("span", { class: "outside" }, h("span", { class: "middle" }, h("span", { class: "inside" }, items.map(item => {
            const segment = item.segment;
            if (segment === 'sign' || segment === 'symbol') {
                const itemZoom = baseZoom * (item.zoom ?? 1);
                const pad = (item.padding ?? 0) * itemZoom;
                const width = (item.width ?? 0) * itemZoom + pad * 2;
                const height = (item.height ?? 0) * itemZoom + pad * 2;
                const lane = item.lane ?? 0;
                const minX = item.minX ?? 0;
                const right = (1000 - minX * 2 - (item.width ?? 0)) * itemZoom - offset * baseZoom * lane;
                const padding = Math.max(0, 20 * itemZoom - pad);
                const style = {
                    '--sgnw-height': `${height}px`,
                    width: `${width}px`,
                    height: `${height}px`,
                    marginRight: `${Math.max(0, right)}px`,
                    borderLeft: right < 0 ? `${-right}px solid transparent` : undefined,
                    padding: `${padding}px`,
                };
                if (segment === 'sign') {
                    return (h("swu-sign", { sign: item.text, styling: this.styling, baseUrl: this.baseUrl, style: style }));
                }
                return (h("swu-symbol", { symbol: this.swuSymbolChar(item.text), styling: this.styling, baseUrl: this.baseUrl, style: style }));
            }
...
SwuVp.style = swuVpCss();

export { SwuVp as swu_vp };
