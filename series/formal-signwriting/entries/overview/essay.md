...
This overview is a map. It is not the full argument for the model.
...
This overview has four jobs:

- name the technical layers
- show the reading order
- keep canonical text, rendering, styling, and implementation practice distinct
- point technical readers toward the right companion material

The series should not be read as one long specification where every entry repeats the same facts.

The intended editorial split is:
...
Each entry should stand alone, but the series is strongest when read as a layered technical canon.
