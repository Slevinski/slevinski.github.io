# Sutton SignWriting Infrastructure and Program

*Stewardship and program-design briefs for continuity, host-structure requirements, international participation, shared infrastructure, and layered AI/data governance*

This repository contains the stewardship, continuity, and program-design series of the Sutton SignWriting platform. It distinguishes what already exists, what continuity risks remain, and what technical, human, institutional, data-governance, and funding structures would make the work durable, citable, maintainable, and cumulative.

The central stewardship question is:

> what structure lets a real international language infrastructure survive, improve, and support research?

## Repository Details

- Repository: `sutton-signwriting/infrastructure-and-program`
- Living site: `https://steveslevinski.me/#series/infrastructure-and-program`
- Version: `v1.0.0`
- Publication year: `2026`
- License: `CC BY 4.0`
- Series DOI: `https://doi.org/10.5281/zenodo.20075076`

## Who This Repository Is For

- PIs and research administrators
- university units, institutes, nonprofits, and consortia
- research infrastructure collaborators
- funders and proposal collaborators
- international partners and local coordinators
- future stewards of Sutton SignWriting infrastructure

## How To Read This Series

Begin with:

1. [Sutton SignWriting Infrastructure and Program Overview](entries/overview/essay.md)
2. [Executive Brief](entries/executive-brief/essay.md)
3. [Infrastructure and Stewardship](entries/infrastructure-and-stewardship/essay.md)
4. [Host Structure and Stewardship Requirements](entries/host-structure-and-stewardship-requirements/essay.md)
5. [Infrastructure Core and Philanthropy](entries/infrastructure-core-and-philanthropy/essay.md)
6. [International Shared Support](entries/international-shared-support/essay.md)
7. [Country Ambassadors](entries/country-ambassadors/essay.md)
8. [Relevant Precedents for SignWriting Infrastructure](entries/relevant-precedents-for-signwriting-infrastructure/essay.md)
9. [SignWriting, AI, and Layered Stewardship](entries/signwriting-ai-and-layered-stewardship/essay.md)

## Contents

- [Sutton SignWriting Infrastructure and Program Overview](entries/overview/essay.md) - frames the full series
- [Executive Brief](entries/executive-brief/essay.md) - shortest forwardable stewardship case
- [Infrastructure and Stewardship](entries/infrastructure-and-stewardship/essay.md) - asset base, continuity risks, and practical stewardship functions
- [Host Structure and Stewardship Requirements](entries/host-structure-and-stewardship-requirements/essay.md) - accountable host, consortium, fiscal sponsor, or related structure requirements
- [Infrastructure Core and Philanthropy](entries/infrastructure-core-and-philanthropy/essay.md) - staffing, translation, coordination, grants, gifts, donations, and continuity capacity
- [International Shared Support](entries/international-shared-support/essay.md) - international participation without inappropriate obligations
- [Country Ambassadors](entries/country-ambassadors/essay.md) - bounded local liaison role for outreach, translation, relationship-building, and follow-up
- [Relevant Precedents for SignWriting Infrastructure](entries/relevant-precedents-for-signwriting-infrastructure/essay.md) - archive, language, data, and research-infrastructure models
- [SignWriting, AI, and Layered Stewardship](entries/signwriting-ai-and-layered-stewardship/essay.md) - layer map for written signs, records, media, datasets, encodings, and AI deployment
- [Publishing Notes](publishing.md) - release metadata, citation posture, and artifact register

## Citation

```text
Slevinski, S. (2026). Sutton SignWriting Infrastructure and Program (v1.0.0). Zenodo. https://doi.org/10.5281/zenodo.20075076
```

Use the series DOI when citing the stewardship series as a whole. Use an artifact DOI when citing a specific brief or white paper that has its own record.

## Platform Context

This repository is one of eight series in the Sutton SignWriting platform release. It is explicit about continuity requirements without overstating secured capacity or implying sponsorship that does not formally exist.

## License

Unless otherwise noted, the contents are released under the Creative Commons Attribution 4.0 International License (`CC BY 4.0`).
