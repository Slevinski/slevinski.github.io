# University of Iowa

*A presentation and handout series for University of Iowa alignment, outreach, and collaborator context*

This series is for materials tailored to University of Iowa faculty, grants staff, development staff, and prospective academic collaborators.

The series is designed to answer one practical question from several different angles:

> what is the strongest next structure for supporting Steve Slevinski's immediate value and longer-term infrastructure opportunity at the University of Iowa?

The decks are not interchangeable.

Each one has a specific job, audience, and level of detail.

## Current entries

- **Outreach Memo** — the shortest bridge deck for forwarding internally, summarizing the whole structure and routing the next conversation
- **Immediate Lab Home** — the PI-facing first-step deck that argues for a lab home now as the small decision that unlocks continuity, support, and future growth
- **Steve Slevinski Value** — the immediate operational-value deck focused on DMS, workflow support, code, structure, and repository delivery
- **Sutton SignWriting Infrastructure Initiative** — the longer-horizon infrastructure deck focused on existing global value, current fragility, and the case for durable institutional stewardship
- **Infrastructure Core and Philanthropy** — the philanthropic and development-facing deck focused on establishing a durable institutional core for staffing, continuity, and global coordination
- **Relevant Precedents and Institutional Pathways** — the precedent-focused deck showing how comparable university, archive, and sign-language models support the Iowa path

## Series role

This series should answer practical University of Iowa questions:

- why this work belongs here
- what support could look like
- who is leading the work technically
- why the initiative is credible and timely
- how the immediate lab-home need, project value, and long-term infrastructure fit together

## Recommended order of use

The strongest default order is:

1. **Outreach Memo** for internal forwarding and first-pass context
2. **Immediate Lab Home** for a PI or near-term placement conversation
3. **Steve Slevinski Value** for salary support, leadership, or collaborator conversations centered on practical usefulness
4. **Sutton SignWriting Infrastructure Initiative** once the practical base is established and the longer-term opportunity is under discussion
5. **Infrastructure Core and Philanthropy** when the conversation shifts toward development, donor framing, and the case for durable philanthropic support
6. **Relevant Precedents and Institutional Pathways** when the conversation needs external examples, comparable models, or evidence that the proposal follows a recognizable institutional path
